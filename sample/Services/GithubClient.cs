...
public class GithubClient
{
    ...
    public GithubClient(CLILogger logger, HttpClient httpClient, RetryPolicy retryPolicy, DateTimeProvider dateTimeProvider, string personalAccessToken)
    {
        _logger = logger;
        ...
        Dictionary<string, string> customHeaders = null)
    {
        ...
        _logger.LogVerbose($"GITHUB REQUEST ID: {ExtractHeaderValue("X-GitHub-Request-Id", response.Headers)}");
    ...
    private async Task ApplyRetryDelayAsync()
    {
        if (_retryDelay > 0)
        {
            _logger.LogWarning($"GitHub rate limit exceeded. Waiting {_retryDelay} seconds before continuing");
    ...
    private void EnsureSuccessGraphQLResponse(JObject response)
    {
        if (response.TryGetValue("errors", out var jErrors) && jErrors is JArray { Count: > 0 } errors)
        {
            ...
            throw new CLIException($"{errorMessage ?? "UNKNOWN"}");
        }
    }
