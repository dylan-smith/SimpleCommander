...
public class AzureApi
{
    ...
    public AzureApi(HttpClient client, BlobServiceClient blobServiceClient, CLILogger log)
    {
        _client = client;
    ...
    {
        _logger.LogVerbose($"HTTP GET: {fromUrl}");
        using var response = await _client.GetAsync(fromUrl);
        _logger.LogVerbose($"RESPONSE ({response.StatusCode}): <truncated>");
