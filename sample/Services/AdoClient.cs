...
public class AdoClient
{
    ...
    public AdoClient(CLILogger log, HttpClient httpClient, RetryPolicy retryPolicy, string personalAccessToken)
    {
        _logger = log;
    ...
    private async Task ApplyRetryDelayAsync()
    {
        if (_retryDelay > 0.0)
        {
            _logger.LogWarning($"THROTTLING IN EFFECT. Waiting {(int)_retryDelay} ms");
    ...
    public virtual async Task<JArray> GetWithPagingAsync(string url, string continuationToken)
    {
        ...
        await ApplyRetryDelayAsync();
        _logger.LogVerbose($"HTTP GET: {url}");

        var response = await _retryPolicy.HttpRetry(async () =>
        {
            var httpResponse = await _httpClient.GetAsync(url);
            var httpContent = await httpResponse.Content.ReadAsStringAsync();
            _logger.LogVerbose($"RESPONSE ({httpResponse.StatusCode}): {httpContent}");
