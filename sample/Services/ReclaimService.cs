...
public class ReclaimService
{
    ...
    public ReclaimService(GithubApi githubApi, CLILogger logger)
    {
    ...
    public virtual async Task ReclaimMannequin(string mannequinUser, string mannequinId, string targetUser, string githubOrg, bool force, bool skipInvitation)
    {
        ...
        {
            throw new CLIException($"User {mannequinUser} is not a mannequin.");
        }
        ...
        {
            throw new CLIException($"User {mannequinUser} is already mapped to a user. Use the force option if you want to reclaim the mannequin again.");
        }
        ...
        if (skipInvitation)
        {
            foreach (var mannequin in mannequins.GetUniqueUsers())
            {
                ...
                {
                    throw new CLIException("Failed to reclaim mannequin.");
                }
            }
        ...
        else
        {
            ...
            if (!success)
            {
                throw new CLIException("Failed to send reclaim mannequin invitation(s).");
            }
        }
    ...
    public virtual async Task ReclaimMannequins(string[] lines, string githubTargetOrg, bool force, bool skipInvitation)
    {
        ...
        if (lines.Length == 0)
        {
            _logger.LogWarning("File is empty. Nothing to reclaim");
            return;
        }
        ...
        {
            throw new CLIException($"Invalid Header. Should be: {CSVHEADER}");
        }
        ...
        foreach (var mannequin in parsedMannequins)
        {
            ...
            {
                _logger.LogWarning($"{mannequin.Login} is already claimed. Skipping (use force if you want to reclaim)");
                continue;
            }

            if (mannequins.FindFirst(mannequin.Login, mannequin.Id) == null)
            {
                _logger.LogWarning($"Mannequin {mannequin.Login} not found. Skipping.");
                continue;
            }

            if (parsedMannequins.Where(x => x.Login == mannequin.Login && x.Id == mannequin.Id).Count() > 1)
            {
                _logger.LogWarning($"Mannequin {mannequin.Login} is a duplicate. Skipping.");
                continue;
            }
            ...
            catch (CLIException ex) when (ex.Message.Contains("Could not resolve to a User with the login"))
            {
                _logger.LogWarning($"Claimant \"{mannequin.MappedUser.Login}\" not found. Will ignore it.");
                continue;
            }
    ...
    private bool HandleInvitationResult(string mannequinUser, string targetUser, Mannequin mannequin, string targetUserId, CreateAttributionInvitationResult result)
    {
        ...
        {
            _logger.LogError($"Failed to send reclaim invitation email to {targetUser} for mannequin {mannequinUser} ({mannequin.Id}): {result.Errors[0].Message}");
            return false;
        }
        ...
        {
            _logger.LogError($"Failed to send reclaim invitation email to {targetUser} for mannequin {mannequinUser} ({mannequin.Id})");
            return false;
        }

        _logger.LogInformation($"Mannequin reclaim invitation email successfully sent to {targetUser} for {mannequinUser} ({mannequin.Id})");
    ...
    private bool HandleReclaimationResult(string mannequinUser, string targetUser, Mannequin mannequin, string targetUserId, ReattributeMannequinToUserResult result)
    {
        if (result.Errors != null)
        {
            ...
            switch (result.Errors[0].Message)
            {
                case string a when a.Contains("is not an Enterprise Managed Users (EMU) organization"):
                    _logger.LogError("Failed to reclaim mannequins. The --skip-invitation flag is only available to EMU organizations.");
                    return false; // Indicates we should stop parsing through the CSV
                default:
                    _logger.LogWarning($"Failed to reattribute content belonging to mannequin {mannequinUser} ({mannequin.Id}) to {targetUser}: {result.Errors[0].Message}");
                    return true;
            }
        }
            ...
            result.Data.ReattributeMannequinToUser.Target.Id != targetUserId)
        {
            ...
            _logger.LogWarning($"Failed to reattribute content belonging to mannequin {mannequinUser} ({mannequin.Id}) to {targetUser}");
            return true;
        }

        _logger.LogInformation($"Successfully reclaimed content belonging to mannequin {mannequinUser} ({mannequin.Id}) to {targetUser}");
    ...
    private (string MannequinUser, string MannequinId, string TargetUser) ParseLine(string line)
    {
        ...
        {
            _logger.LogWarning($"Invalid line: \"{line}\". Will ignore it.");
        ...
        {
            _logger.LogWarning($"Invalid line: \"{line}\". Mannequin login is not defined. Will ignore it.");
            return (null, null, null);
        }

        if (string.IsNullOrEmpty(userId))
        {
            _logger.LogWarning($"Invalid line: \"{line}\". Mannequin Id is not defined. Will ignore it.");
            return (null, null, null);
        }

        if (string.IsNullOrEmpty(claimantLogin))
        {
            _logger.LogWarning($"Invalid line: \"{line}\". Target User is not defined. Will ignore it.");
            return (null, null, null);
        }
