...
public class AdoInspectorService
{
    ...
    public AdoInspectorService(CLILogger logger, AdoApi adoApi)
    {
        _logger = logger;
        _adoApi = adoApi;
    }
    ...
    public virtual async Task<IEnumerable<string>> GetOrgs()
    {
        if (_orgs is null)
        {
            ...
            {
                _logger.LogInformation($"Retrieving list of all Orgs PAT has access to...");
    ...
    public virtual void OutputRepoListToLog()
    {
        ...
        {
            _logger.LogInformation($"ADO Org: {org}");

            foreach (var teamProject in _repos[org].Keys)
            {
                _logger.LogInformation($"  Team Project: {teamProject}");
